...
class GPT:
    ...
    def __call__(self, prompt, n:int=1, **kwargs: Any) -> Any:    #* n: How many chat completion choices to generate for each input message.
        return self.call_wrapper(prompt, n, self.model_name, self.T, self.seed, **kwargs)
        
    def call_wrapper(self, prompt, n, model, temperature, seed, **kwargs: Any) -> Any:
...
class AzureGPT4o:
    ...
    def __call__(self, prompt, n:int=1, **kwargs: Any) -> Any:    #* n: How many chat completion choices to generate for each input message.
        ...
        if self.model_name in ["azure-gpt4o", "azure-gpt4o-mini", "azure-gpt4", "azure-gpt35-turbo"]:
            return self.call_wrapper(messages=prompt, n=n, model=azure_deploy_name, temperature=self.T, seed=self.seed, **kwargs)
        elif self.model_name in ["azure-openai-o1", "azure-openai-o1-mini", "azure-openai-o3-mini"]:
            return self.call_wrapper(messages=prompt, n=n, model=azure_deploy_name, seed=self.seed, **kwargs)
        else:
...
class AnthropicClaude:
    ...
    def __call__(self, prompt, n=1, **kwargs: Any) -> Any:
    ...
    def call_wrapper(self, prompt, model, temperature, **kwargs: Any) -> Any:
        ...
        while retry_count < retries:
            try:
                ...
                response = self.client.messages.create(messages=message, model=model, temperature=temperature, max_tokens=2048, **kwargs) 
                break
            except Exception as e:
                retry_count += 1
                backoff_time = 2**retry_count+1
...
class ZhipuLLM:
    ...
    def __call__(self, prompt, n:int=1, **kwargs: Any) -> Any:    #* n: How many chat completion choices to generate for each input message.
        return self.call_wrapper(prompt, n, self.model_name, self.T, **kwargs)
    
    def call_wrapper(self, prompt, n, model, temperature, **kwargs: Any) -> Any:
        retry_count, retries = 0, 5
        response = None
        while retry_count < retries:
            try:
                message = [{'role':'user', 'content':prompt}] if 'system' not in kwargs else [{'role':'system', 'content':kwargs['system']}, {'role':'user', 'content':prompt}] 
                if 'system' in kwargs: kwargs.pop('system') 
                response = self.client.chat.completions.create(messages=message, model=model, temperature=temperature, max_tokens=2048, top_p=1.0, **kwargs)
                break
            except Exception as e:
                retry_count += 1
                backoff_time = 2**retry_count+1
...
class BaseAPIModels:
    ...
    def __call__(self, prompt, n=1, top_p=0.9, **kwargs: Any) -> Any:
        
        prompt = [{'role':'user', 'content':prompt}] if 'system' not in kwargs else [{'role':'system', 'content':kwargs['system']}, {'role':'user', 'content':prompt}] 
        if self.provider_name == 'tencent' and 'r1' in self.model_name:                                                 # tencent's deepseek-r1 does not support `top_p`
            response = self.call_wrapper(messages=prompt, n=n, model=self.model_DI, temperature=self.T, **kwargs)
        else:
